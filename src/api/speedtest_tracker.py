...
def build_summary(cfg: SpeedtestTrackerConfig, net: NetworkConfig) -> Dict[str, Any]:
    ...
    # Since the API forces pagination (25 items/page), we must loop until we reach this date.
    ...
    try:
        with httpx.Client(base_url=str(cfg.url).rstrip("/"), headers=headers, verify=False, timeout=net.timeout) as c:
            ...
            # Safety break: Don't loop forever if DB is huge (limit to ~50 pages / 1250 items)
            for _ in range(50):
                ...
                for item in items:
                    try:
                        # 1. Parse Time (Naive for comparison)
                        dt = parser.parse(item['created_at']).replace(tzinfo=None)
                        batch_oldest = dt
                        
                        # 2. Parse Values (Handle Bytes -> Bits conversion)
                        # Prefer explicit 'bits' fields if available (Speedtest Tracker v0.11+)
                        if 'download_bits' in item:
                            dl_bits = float(item['download_bits'])
                            ul_bits = float(item['upload_bits'])
                        else:
                            # Fallback: API provides Bytes, we need Bits
                ...
                # PREPARE NEXT PAGE
                # We must scrape the 'next' link because the API uses specific array syntax (page[number])
